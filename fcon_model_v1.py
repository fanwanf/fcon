"""
Original FCON model (paper / GitHub v1) — 5-channel input, no background depth, no normals.

This is identical to the model in https://github.com/nikhilmishra000/fcon and is used
to load the author-provided checkpoint checkpoints/fcon.pt for comparison.

Class is named FCON_V1 to coexist with the modified FCON in fcon_model.py.
"""
import attr
import numpy as np
import torch
...
class FCON_V1(nn.Module):
    ...
    def __attrs_post_init__(self):
        ...
        # 3D-UNet — original 5-channel input
    ...
    def _get_frustums(self, near_plane, far_plane, point_map, intrinsic,
                      boxes, masks, im_size, perturb):
        boxes = enlarge_box_roi(boxes, im_size, 1.2)
        n_objects = len(boxes)
        near_planes = (
            expand_dim(point_map[2], dim=0, shape=[n_objects])
            .masked_fill(~masks, np.inf)
            .flatten(1, 2).min(1).values
        )
        ...
        if perturb:
            near_plane_offset = (
                torch.randn_like(near_planes).mul(0.1).add(near_plane_offset).clip(0.05, 0.2)
            )
            far_plane_offset = torch.rand_like(far_planes).sub(0.5).mul(0.2)
        near_planes = near_planes - near_plane_offset
        far_planes  = far_planes  - far_plane_offset
        return Frustum(roi=boxes, near_plane=near_planes,
                       far_plane=far_planes, intrinsic=intrinsic)

    def _voxelize_roi_point_clouds(self, frustums, point_map):
        rois  = F.pad(frustums.roi, (1, 0), value=0.0)
        depth = torchvision.ops.roi_align(
            point_map[None, None, 2], rois,
            output_size=(self.patch_size, self.patch_size), sampling_ratio=2,
        ).squeeze(1)
        eps = 1e-3
        ...
        return F.one_hot(z_idx, self.n_depth_bins).permute(0, 3, 1, 2).bool()

    def _compute_roi_features(self, rgb, intrinsic, point_map, boxes, masks,
                               near_plane, far_plane, perturb=False):
        device = rgb.device
        im_size = torch.tensor(rgb.shape[-2:], device=device)
        frustums = self._get_frustums(near_plane, far_plane, point_map, intrinsic,
                                      boxes, masks, im_size, perturb=perturb)
        voxelized = self._voxelize_roi_point_clouds(frustums, point_map)
        ...
        masks_p = torchvision.ops.roi_align(
            masks[:, None].float(), mask_rois,
            output_size=(self.patch_size, self.patch_size), sampling_ratio=2,
        )
        roi_feats = torch.cat([rgb_p, masks_p, masks_p], dim=1)  # 5-channel
        roi_voxelized_features = torch.einsum("bdhw,bchw->bcdhw", voxelized, roi_feats)
        return roi_voxelized_features, grid_pts_cam

    def forward(self, roi_voxelized_features):
        feats = self.feature_module(roi_voxelized_features).squeeze(1)
        feats = self.unet2d(feats)
        return self.predictor(feats)

    def predict(self, rgb, intrinsic, point_map, boxes, masks,
                near_plane, far_plane, perturb=False):
        ...
        return {"logits": logits, "grid_centers": grid_pts_cam}
